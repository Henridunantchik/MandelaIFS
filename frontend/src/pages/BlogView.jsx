...
const BlogView = () => {
    ...
    const handleShare = (blogId) => {
      ...
      useEffect(()=>{
        // Enhance links inside rendered content: force-download for Cloudinary docs
        const el = contentRef.current;
        if (!el) return;
        const anchors = el.querySelectorAll('a[href]');
        anchors.forEach(a => {
          const href = a.getAttribute('href') || '';
          if (/^https?:\/\/res\.cloudinary\.com\//.test(href) && href.includes('/upload/') && !/\/upload\/.*fl_attachment/.test(href)) {
            const forced = href.replace('/upload/', '/upload/fl_attachment/');
            a.setAttribute('href', forced);
    ...
    return (
        <div className='pt-14'>
            <div className='max-w-6xl mx-auto p-10'>
                ...
                <div ref={contentRef} className='blog-content' dangerouslySetInnerHTML={{ __html: selectedBlog.description }} />

                <div className='mt-10'>
                    {/* Tags */}
                    <div className="flex flex-wrap gap-2 mb-8">
                        {selectedBlog?.mainCategory && (
                          <Badge variant="secondary">{selectedBlog.mainCategory}</Badge>
                        )}
                        {(selectedBlog?.subcategories||[]).map((t, idx)=> (
                          <Badge key={`sub-${idx}`} variant="secondary">{t}</Badge>
                        ))}
                        {(selectedBlog?.transversalTags||[]).map((t, idx)=> (
                          <Badge key={`tr-${idx}`} variant="secondary">{t}</Badge>
                        ))}
                    </div>
